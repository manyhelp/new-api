...
> **📌 注意事项：**
> - 参数格式如上示例，`apiId` 字段固定必传，值为 `752`，请勿修改.
...
| 参数序号 | 参数名 | 类型 | 必传 | 说明 |
|----------|--------|------|------|------|
| 0 | `prompt` | string | 是 | 描述想要生成的内容，支持联网搜索获取最新信息，文本内容，最大长度 `2500` 字符 |
| 1 | `model_name` | string | 是 | 模型，固定值 `cinema-generate-2.0`，不可修改 |
| 2 | `duration` | string | 是 | 视频参数。可选值：`4`(4s),`5`(5s),`6`(6s),`7`(7s),`8`(8s),`9`(9s),`10`(10s),`11`(11s),`12`(12s),`13`(13s),`14`(14s),`15`(15s) |
| 3 | `mode` | string | 是 | 视频参数。可选值：`480p`(480p),`720p`(720p),`1080p`(1080p) |
| 4 | `aspect_ratio` | string | 是 | 宽高比。可选值：`16:9`(16:9),`9:16`(9:16),`4:3`(4:3),`1:1`(1:1),`3:4`(3:4),`21:9`(21:9) |
| 5 | `generate_audio` | boolean | 是 | 同步音频。可选值：`true`(开),`false`(关) |
| 6 | `tools` | boolean | 否 | 联网搜索。可选值：`true`(开),`false`(关), 默认值：`false` |
